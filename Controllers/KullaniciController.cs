	...
	public IActionResult Login([FromForm] LoginViewModel model)
	{
		if (ModelState.IsValid)
		{
			using (var db = new YemekDbContext())
			{
				...
				if (kullanici.Any())
				{
					if (string.IsNullOrEmpty(HttpContext.Session.GetString("Isim")))
					{
						HttpContext.Session.SetString("Id", kullanici.FirstOrDefault().Id.ToString());
						HttpContext.Session.SetString("Isim", kullanici.FirstOrDefault().Isim.ToString());
					}
					TempData["Isim"] = HttpContext.Session.GetString("Isim").ToString();
